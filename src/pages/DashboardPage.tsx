...
const stats = [
  ...
  { label: 'Total Tickets', value: '234', icon: Ticket, color: 'text-purple-400', bg: 'bg-purple-500/10' },
];
...
const channels = [
  ...
  { id: 'c2', name: 'GoldTips Free', type: 'free', price: null, subscribers: 48200, revenue: '₦0', active: true },
];
...
export function DashboardPage() {
  ...
  const [ticketMatches, setTicketMatches] = useState([
    { home: '', away: '', odds: '', sport: 'Football' }
  ]);
  const [ticketSport, setTicketSport] = useState('Football');

  const addMatch = () => {
    setTicketMatches(prev => [...prev, { home: '', away: '', odds: '', sport: ticketSport }]);
  };

  const removeMatch = (i: number) => {
    setTicketMatches(prev => prev.filter((_, idx) => idx !== i));
  };
          ...
          <div className="flex items-center gap-2 mb-3">
            <div>
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="text-xs text-[#ef4444] font-bold">Verified Tipster</span>
              </div>
            </div>
          </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button
                    ...
                    className="text-xs text-[#ef4444] font-bold hover:underline flex items-center gap-1"
                  >
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        {ch.type === 'paid'
                          ? <span className="text-[9px] bg-yellow-500/20 text-yellow-400 px-1.5 py-0.5 rounded-full font-bold">VIP · {ch.price}</span>
                          : <span className="text-[9px] bg-green-500/20 text-green-400 px-1.5 py-0.5 rounded-full font-bold">FREE</span>
                        }
                        {ch.active && <span className="text-[9px] bg-green-500/20 text-green-400 px-1.5 py-0.5 rounded-full font-bold">● ACTIVE</span>}
                      </div>
                    </div>
              ...
              <div>
                ...
                <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-1">
                  {sports.map(sport => (
                    <button
                      key={sport}
                      onClick={() => setTicketSport(sport)}
                      className={cn(
                        'px-3 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-all shrink-0',
                        ticketSport === sport
                          ? 'bg-[#ef4444] text-white'
                          : 'bg-[#111] border border-[#1f1f1f] text-[#71767b] hover:text-white'
                      )}
                    >
                      {sport}
                    </button>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      {ticketMatches.length > 1 && (
                        <button onClick={() => removeMatch(i)} className="text-[#ef4444] hover:text-red-300">
              ...
              <div>
                ...
                <div className="space-y-2">
                  {channels.map(ch => (
                    <button
                      key={ch.id}
                      className="w-full flex items-center gap-3 p-3 bg-[#111] border border-[#1f1f1f] hover:border-[#ef4444]/30 rounded-xl transition-all"
                    >
          ...
          {activeTab === 'payouts' && (
            <div className="p-4 space-y-4">
              ...
              <div>
                ...
                <div className="space-y-2">
                  {[
                    { amount: '₦5,000,000', date: 'Apr 1, 2026', status: 'paid' },
                    { amount: '₦4,200,000', date: 'Mar 1, 2026', status: 'paid' },
                    { amount: '₦3,800,000', date: 'Feb 1, 2026', status: 'paid' },
                    { amount: '₦2,500,000', date: 'Jan 1, 2026', status: 'paid' },
                  ].map((payout, i) => (
                    <motion.div
                      key={i}
                      initial={{ opacity: 0, y: 4 }}
              ...
              <div className="bg-[#111] border border-[#1f1f1f] rounded-2xl p-4">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-[#71767b]">GoldTips VIP Ltd</p>
                </div>
              </div>
